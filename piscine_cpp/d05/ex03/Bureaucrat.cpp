...
/*   Updated: 2015/11/10 21:07:54 by ggilaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				Bureaucrat::executeForm(AForm &F)
{
	...
	{
		F.execute(*this);
		std::cout << this->_name << " has executed " << F.getName() << std::endl;
	}
	catch (AForm::FormNotSigned &e)
		{
			std::cout << F.getName()
				<< " is not signed and bureaucrats follow rules'"  << std::endl;
		}
	catch (AForm::GradeTooLowException &e)
		{
			std::cout << this->_name
				<< " is under-graded, he cannot execute task : '" 
				<< F.getName() << "'" << std::endl;
		}
}
