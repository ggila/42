...
/*   Updated: 2015/04/01 18:19:44 by ggilaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_sort_merge(t_merge tosort, void *aux, int a, int b)
{
	int mid;
	...
	if (b <= a)
		return ;
	mid = (a + b) / 2;
	ft_sort_merge(tosort, aux, a, mid);
	ft_sort_merge(tosort, aux, mid + 1, b);
						...
						int last_index)
{
	...
	if (!(aux = ft_memalloc(type_size * (last_index + 1))))
		return ;
	ft_sort_merge(tosort, aux, 0, last_index);
}
